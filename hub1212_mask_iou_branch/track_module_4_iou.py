...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            outputs_class, outputs_mask, outputs_iou = self.prediction(ms_output, mask_features[:, i, ...])
            out_dict = {
                "pred_logits": outputs_class[-1],  # b, q, k+1
                "pred_masks": outputs_mask[-1],  # b, q, h, w
                "pred_iou": outputs_iou[-1],  # b, q, 1
                "num_new_ins": num_valid_fq.item()
            }

            # mask_iou targets
            _gt_mask_iou = matcher.get_gt_mask_iou(out_dict, targets_i)
            out_dict.update({
                "gt_mask_iou": _gt_mask_iou,
                "using_thr": using_thr,
            })
            ...
            out_dict.update({
                "aux_outputs": self._set_aux_loss(outputs_class, outputs_mask, outputs_iou),
                "disappear_tgt_id": -10000 if self.disappear_tgt_id is None else self.disappear_tgt_id,
            })

            # mask_iou targets
            for aux_idx, aux_out_i in enumerate(out_dict["aux_outputs"]):
                gt_mask_iou_list = matcher.get_gt_mask_iou(aux_out_i, targets_i)
                out_dict["aux_outputs"][aux_idx].update({
                    "gt_mask_iou": gt_mask_iou_list,
                    "using_thr": using_thr,
                })
    ...
    def inference(self, frame_embeds_no_norm, mask_features, frames_info, start_frame_id, resume=False, to_store="cpu"):
        ...
        for i in range(T):
            ...
            ms_output = torch.stack(ms_output, dim=0)  # (num_layers+1, q, b, c)
            outputs_class, outputs_mask, outputs_iou = self.prediction(ms_output[-1:], mask_features[:, i, ...])
    ...
    def prediction(self, outputs, mask_features):
        ...
        outputs_iou = self.iou_embed(decoder_output)      # l, b, q, 1
        outputs_mask = torch.einsum("lbqc,bchw->lbqhw", mask_embed, mask_features)

        return outputs_class, outputs_mask, outputs_iou
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_cls, outputs_mask, outputs_iou):
        return [{"pred_logits": a,
                 "pred_masks": b,
                 "pred_iou": c,
                 "disappear_tgt_id": -10000 if self.disappear_tgt_id is None else self.disappear_tgt_id,
                 } for a, b, c
                in zip(outputs_cls[:-1], outputs_mask[:-1], outputs_iou[:-1])]