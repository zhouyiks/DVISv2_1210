...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                if self.prev_frame_indices is not None and len(self.prev_frame_indices[0]) > 2:
                    select_idx = random.randrange(0, len(self.prev_frame_indices[0]))
                    select_tgt_id = self.prev_frame_indices[1][select_idx]
                    if not using_thr or random.random() < 0.5:
                        self.disappear_tgt_id = None
                        self.disappear_trcQ_id = None
                    elif select_tgt_id in frames_info["indices"][i][0][1]:
                ...
                valid_appear_fq_mask = ~disappear_fq_mask if self.disappear_tgt_id is None else valid_fq_mask & (~disappear_fq_mask)
                ...
                num_trcQ = self.track_queries.shape[0]
                pilot = torch.cat([self.track_queries, new_ins_embeds], dim=0)
                pilot_pos = torch.cat([self.track_embeds, detQ_pos], dim=0)
                disappear_embeds = disappear_embed.repeat(num_trcQ, 1, 1)
                attn_mask = torch.zeros(size=(B, self.num_heads, pilot.shape[0], fQ + num_trcQ),
                                        dtype=torch.bool).to("cuda")
                attn_mask[:, :, :num_trcQ, :fQ] = ~valid_appear_fq_mask[None, None, None, :].repeat(
                    B, self.num_heads, num_trcQ, 1)
                attn_mask[:, :, num_trcQ:, fQ:] = True
                attn_mask = attn_mask.flatten(0, 1)

                self_attn_mask = torch.zeros(size=(B, self.num_heads, pilot.shape[0], pilot.shape[0]),
                                             dtype=torch.bool).to("cuda")
                self_attn_mask[:, :, :num_trcQ, num_trcQ:] = True
                ...
                for j in range(self.num_layers):
                    ...
                    output = self.transformer_self_attention_layers[j](output, tgt_mask=self_attn_mask)
                    output = self.transformer_ffn_layers[j](output)
                    ms_output.append(output)
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                attn_mask = torch.zeros(size=(B, self.num_heads, pilot.shape[0], fQ + self.track_queries.shape[0]),
                                        dtype=torch.bool).to("cuda")
                # attn_mask[:, :, :self.track_queries.shape[0], :fQ] = ~(valid_fq_mask[None, None, None, :].
                #                                                        repeat(B, self.num_heads, self.track_queries.shape[0], 1))
                attn_mask[:, :, self.track_queries.shape[0]:, fQ:] = True
                attn_mask = attn_mask.flatten(0, 1)

                self_attn_mask = torch.zeros(size=(B, self.num_heads, pilot.shape[0], pilot.shape[0]),
                                             dtype=torch.bool).to("cuda")
                self_attn_mask[:, :, :self.track_queries.shape[0], self.track_queries.shape[0]:] = True
                self_attn_mask = self_attn_mask.flatten(0, 1)
    ...
    def get_mask_pos_embed(self, mask, mask_features):
        ...
        if len(pos_embeds_list) == 0:
            return torch.zeros(size=(0, 1, self.hidden_dim)).to(mask_features)
