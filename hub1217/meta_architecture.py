...
from .matcher import FrameMatcher, AuxHungarianMatcher, ConsecutiveFramesMatcher
...
class ViHub_online(MinVIS):
    ...
    def from_config(cls, cfg):
        ...
        reid_weights = cfg.MODEL.VIDEO_HEAD.REID_WEIGHTS
        aux_reid_weights = cfg.MODEL.VIDEO_HEAD.AUX_REID_WEIGHTS
        mask_iou_weight = cfg.MODEL.VIDEO_HEAD.MASK_IOU_WEIGHTS
        ...
        aux_matcher = AuxHungarianMatcher(
            cost_class=class_weight,
            cost_dice=dice_weight,
            cost_mask=mask_weight,
            num_points=cfg.MODEL.MASK_FORMER.TRAIN_NUM_POINTS,
        ...
        weight_dict = {"loss_ce": class_weight, "loss_mask": mask_weight, "loss_dice": dice_weight,
                       "loss_bce": cfg.MODEL.VIDEO_HEAD.VALID_SCORE_LOSS_WEIGHT,
                       "loss_reid": reid_weights, "loss_aux_reid": aux_reid_weights,
                       "loss_disappear": class_weight,
                       "loss_ce_fq": class_weight, "loss_mask_fq": mask_weight, "loss_dice_fq": dice_weight,
                       "loss_iou": mask_iou_weight, "loss_sim": reid_weights}
        ...
        # losses = ["labels", "masks", "disappear", "reid"]
        losses = ["labels", "masks"] if cfg.MODEL.VIDEO_HEAD.NUM_REID_HEAD_LAYERS > 0 else ["labels", "masks"]
        ...
        #     cost_dice=dice_weight,
        #     cost_mask=mask_weight,
        #     cost_embed=cfg.MODEL.VIDEO_HEAD.EMBED_SIM_WEIGHT,
        #     num_points=cfg.MODEL.MASK_FORMER.TRAIN_NUM_POINTS,
        #     metric_type=cfg.MODEL.VIDEO_HEAD.CONSECUTIVE_FRAMES_MATCHING_METRIC,
        #     valid_weight=True,
        #     match_score_thr=cfg.MODEL.VIDEO_HEAD.MATCH_SCORE_THR,
        # )
        ...
        tracker = VideoInstanceCutter(
            ...
            inference_select_threshold=cfg.MODEL.VIDEO_HEAD.INFERENCE_SELECT_THRESHOLD,
            reid_hidden_dim=cfg.MODEL.MASK_FORMER.REID_HIDDEN_DIM,
            num_reid_head_layers=cfg.MODEL.VIDEO_HEAD.NUM_REID_HEAD_LAYERS,
        )
        ...
        return {
            ...
            # gradually increase the length of video clip
    ...
    def forward(self, batched_inputs):
        ...
        if self.using_frame_num is None:
            ...
            elif self.increasing_step[0] <= self.iter < self.increasing_step[1]:
                using_frame_num = self.using_frame_num[1]
            else:
                using_frame_num = self.using_frame_num[2]
            images = []
            for video in batched_inputs:
                if using_frame_num <= 0 or using_frame_num > len(video["image"]):
                    using_frame_num = len(video["image"])
                select_fi_list = np.random.choice(range(len(video["image"])), size=using_frame_num, replace=False)
                select_fi_set = np.unique(select_fi_list)
            ...
            if self.iter in [1000, 4000, 5000, 8000, 9999, 11000, 14000, 15000, 16000, 17000, 19000,
                             20000, 24000, 25000, 30000, 40000, 50000, 59000]:
                print(f"iter: {self.iter}, length of images: {len(images)}")
            images = [(x - self.pixel_mean) / self.pixel_std for x in images]
            images = ImageList.from_tensors(images, self.size_divisibility)
            self.iter += 1
        ...
        with torch.no_grad():
            ...
            frame_embeds_no_norm = image_outputs['pred_embds'].clone().detach()  # (b, c, t, q)
        ...
        B, _, T, Q = frame_embeds_no_norm.shape
        video_targets = self.prepare_targets(batched_inputs, images)
        ...
        outputs = self.tracker(frame_embeds_no_norm, mask_features, video_targets, frame_indices_info,
                               self.aux_matcher, using_thr=self.using_thr)
    ...
    def prepare_targets(self, targets, images): # TODO, datamapper match with the function
    ...
    def run_window_inference(self, images_tensor, window_size=30, long_video_start_fidx=-1):
        ...
        for seq_id, ins_seq in self.tracker.video_ins_hub.items():
            ...
            full_masks = torch.full(size=(num_frames, H, W), fill_value=-1).to(torch.float32).to(to_store)
            seq_logits = []
