...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                #TODO do not mask out when using_thr is False
    ...
    def inference(self, frame_embeds_no_norm, mask_features, frames_info, start_frame_id, resume=False, to_store="cpu"):
        ...
        for i in range(T):
            ...
            keep = self.filter_new_ins(outputs_class[-1, 0], outputs_mask[-1, 0],
                                       ms_output[-1, :-self.num_new_ins, 0, :],
                                       single_frame_embeds_no_norm[frames_info["valid"][i][0], 0, :])

            cur_seq_ids = []
            pred_scores = torch.max(outputs_class[-1, 0].softmax(-1)[:, :-1], dim=1)[0]
            valid_queries = (pred_scores > self.inference_select_thr) & keep
    ...
    def detect_disappear(self, trcQ_embeds, srcQ_embeds):
        """
        ...
        pred_logits = self.logit_scale * trcQ_embeds @ srcQ_embeds.T

        # pred_scores = torch.max(pred_logits.softmax(-1)[:, :-1], dim=-1)[0]
        # return pred_scores > a_thr

        return torch.max(pred_logits.softmax(-1), dim=-1)[1] != (pred_logits.shape[1] - 1)

    def filter_new_ins(self, pred_logits, pred_masks, trcQ_embeds, srcQ_embeds):
        """
        ...
        if num_instances == self.num_new_ins:
        ...
        prior_keep = self.detect_disappear(trcQ_embeds, srcQ_embeds)
        ...
        prior_keep = prior_keep[sorted_indices] & (trcQ_scores > self.inference_select_thr)
        trcQ_keep = mask_nms(trcQ_seg_masks, trcQ_scores, self.mask_nms_thr, prior_keep)
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_cls, outputs_mask, ms_outputs, frame_embeds, frame_embeds_indices):
