...
from vihub.config import add_vihub_config
		...
		if len(vid_frames) == windows_size or i == len(frames_path) - 1:
			...
			with autocast():
				if i < windows_size:
					...
					long_video_start_fidx = i - len(vid_frames) + 1
					predictions, visualized_output = demo.run_on_video(vid_frames, keep=True, long_video_start_fidx=long_video_start_fidx)
			# do save
