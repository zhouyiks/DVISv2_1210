...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                # self.track_embeds, _, _, _ = self.get_mask_pos_embed(
                #     self.track_queries, None, ori_mask_features[:, i, ...],
                #     mk=None, a_sq=None, mask_out=mask_out_bg)

                # self.track_embeds, _, _, _ = self.get_mask_pos_embed(
                #     single_frame_embeds_no_norm, frames_info["pred_masks"][i][0][None], ori_mask_features[:, i, ...],
                #     mk=None, a_sq=None, mask_out=mask_out_bg)
                detQ_pos = self.get_mask_pos_embed_average_pooling(frames_info["pred_masks"][i][0][None],
                                                                   ori_mask_features[:, i, ...])
            ...
            if not using_thr:
                ...
                self.track_embeds = self.get_mask_pos_embed_average_pooling(outputs_mask[-1, :, valid_track_query, :, :],
                                                        ori_mask_features[:, i, ...])
            ...
            # track_embeds, _, _, _ = self.get_mask_pos_embed(
            #     ms_output[-1], outputs_mask[-1, ...], ori_mask_features[:, i, ...],
            #     mk=None, a_sq=None, mask_out=mask_out_bg)

            # track_embeds = self.get_mask_pos_embed_average_pooling(outputs_mask[-1, ...], ori_mask_features[:, i, ...])
    ...
    def inference(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, frames_info, start_frame_id, resume=False, to_store="cpu"):
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                detQ_pos, _, curr_mk, curr_a_sq = self.get_mask_pos_embed(
                    single_frame_embeds_no_norm, frames_info["pred_masks"][i][0][None], ori_mask_features[:, i, ...],
                    mk=None, a_sq=None, mask_out=mask_out_bg)
    ...
    def get_mask_pos_embed_average_pooling(self, mask, mask_features):
        """
        mask: b, q, h, w
        mask_features: b, c, h, w
        """
    ...
    def get_mask_pos_embed(self, track_queries, mask_logits, mask_features, mk=None, a_sq=None, mask_out=False):
        ...
        qk = self.mask_embed(qk)
        qk = qk.permute(1, 2, 0)  # b, c, q
        if mk is None or a_sq is None:
            mk = self.mask_feature_proj(mask_features)
            mk = mk.flatten(2)  # b, c, n
            a_sq = mk.pow(2).sum(1).unsqueeze(2)  # b, n, 1

        pos_embeds_list = []
        num_chunk = track_queries.shape[0] // 50 + 1
        for i in range(num_chunk):
            start = i * 50
            end = start + 50 if start + 50 < track_queries.shape[0] else track_queries.shape[0]
        ...
        return self.pos_embed(pos_embeds), pos_embeds, mk, a_sq
