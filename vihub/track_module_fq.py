...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            if not using_thr or self.num_reid_head_layers <= 0:
            ...
            fq_tgt2src_dict = {tgt.item(): src.item() for tgt, src in zip(frames_info["indices"][i][0][1],
                                                                          frames_info["indices"][i][0][0])}
            tq_reid_embeds = self.reid_embed(ms_output[-1])   # q'+nq, b, c
            for tgt, src in fq_tgt2src_dict.items():
                if tgt not in self.gt_ins_hub.keys():
                    self.gt_ins_hub[tgt] = VideoInstanceSequence(0, tgt)
                self.gt_ins_hub[tgt].update(single_frame_reid_embeds[src, 0, :])
            ...
            out_dict.update({
                ...
                "reid_embeds": tq_reid_embeds,
                # "use_disappear_embed_count": use_disappear_embed_count,
                "disappear_tgt_id": -10000 if self.disappear_tgt_id is None else self.disappear_tgt_id,
                "tq_reid_embeds": tq_reid_embeds,
            })
            all_outputs.append(out_dict)
        ...
        for i in range(T):
            ...
            tq_reid_embeds = all_outputs[i]["tq_reid_embeds"]  # q'+nq, b, c
            tq_tgt2src_dict = {tgt.item(): src.item() for tgt, src in zip(all_outputs[i]["indices"][0][1],
                                                                          all_outputs[i]["indices"][0][0])}
            for tgt_id in tq_tgt2src_dict.keys():
                if tgt_id == all_outputs[i]["disappear_tgt_id"]:
                    continue  # TODO, positive to disappear_embed
                anchor_embedding = tq_reid_embeds[tq_tgt2src_dict[tgt_id], 0, :]
                positive_embedding = self.gt_ins_hub[tgt_id].similarity_guided_reid_embed
                negative_query_id = sorted(
                    random.sample(set(range(tq_reid_embeds.shape[0])) - set([tq_tgt2src_dict[tgt_id]]),
                                  tq_reid_embeds.shape[0] - 1)
                )
                reid_out = {
                    "anchor_embedding": anchor_embedding,
                    "positive_embedding": positive_embedding,
                    "negative_embedding": tq_reid_embeds[negative_query_id, 0, :]
                }
