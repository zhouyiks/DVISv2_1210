...
class ViHubCriterion(nn.Module):
    ...
    def loss_sim(self, outputs, targets, indices, num_masks):
        ...
        for output_i, target_i, indice_i in zip(outputs, targets, indices):
            ...
            pos_frame_embeds = frame_embeds[fq_indices[0]]  # nvalid, c
            right_embeds = torch.cat([pos_frame_embeds, disappear_embed], dim=0)  # nvalid + 1, c
            left_embeds = pred_embeds[:-self.num_new_ins, :]

            _num_classes = right_embeds.shape[0]
            gt_idx2label = {t.item(): i for i, t in enumerate(fq_indices[1])}
            if output_i["disappear_tgt_id"] >= 0:
                gt_idx2label.update({output_i["disappear_tgt_id"].item(): _num_classes})
            ...
            for _src_idx, _tgt_idx in zip(src_idx, tgt_idx):
                if _src_idx > len(left_embeds.shape[0]):
                    continue
                tgt_label[_src_idx] = gt_idx2label[_tgt_idx]
            ...
            loss_list.append(F.cross_entropy(pred_logits, tgt_label))
